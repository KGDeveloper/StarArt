...
@interface KGNearConsumptionVC ()<UITableViewDelegate,UITableViewDataSource,MAMapViewDelegate>
/** 筛选条件 */
...
- (void)viewDidLoad {
    ...
    self.typeArr = @[@{@"name":@"美食",@"id":@"7"},@{@"name":@"咖啡",@"id":@"8"},@{@"name":@"茗茶",@"id":@"9"},@{@"name":@"糕点面包",@"id":@"10"},@{@"name":@"集成店",@"id":@"11"},@{@"name":@"电影",@"id":@"12"}];
...
- (void)requestScreenData{
    self.hud = [MBProgressHUD showHUDAddedTo:self.view animated:YES];
    __weak typeof(self) weakSelf = self;
    [KGRequest postWithUrl:FindTypeConsumeMerchant parameters:@{@"longitude":@([self.longStr doubleValue]),@"latitude":@([self.latStr doubleValue]),@"id":self.sencedStr,@"type":self.classStr} succ:^(id  _Nonnull result) {
        [weakSelf.hud hideAnimated:YES];
        if ([result[@"status"] integerValue] == 200) {
            NSDictionary *dic = result[@"data"];
            NSArray *tmp = dic[@"list"];
